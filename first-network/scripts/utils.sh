...
instantiateChaincode() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    ...
    # peer chaincode instantiate -o orderer.example.com:7050 -C mychannel -n mycc -l node -v 1.0 -c '{"Args":["Init","a","100","b","200"]}' -P "AND ('Org1MSP.peer','Org2MSP.peer')"
    res=$?
    set +x
  else
    set -x
...
chaincodeQuery() {
  ...
  do
    ...
    set -x
    peer chaincode query -C $CHANNEL_NAME -n mycc -c '{"Args":["readBloodBag","a"]}' >&log.txt
    res=$?
    set +x
